      ...
      {props.test_history[0] ? ( // checking if props exist (if there are tests for this run)
        <div>
          <Breadcrumbs style={{ paddingLeft: "30px" }}>
            ...
            <Typography color="textPrimary">Passed Tests</Typography>
          </Breadcrumbs>
        ...
        <h1>No passed tests for this run :(( </h1>
      )}
    </BasePage>
  )
}
...
Tests.getInitialProps = async (context): Promise<Props> => {
  const { passedTestsByRunId } = context.query

  // Suites and tests (inside suites)
  const testsByTestRunIdReq = await fetch(
    `${process.env.deltaCore}/api/v1/tests_history/test_status/2/test_run/${passedTestsByRunId}`,
    {
      method: "GET",
    }
  )
  const passedTests = await testsByTestRunIdReq.json()

  return {
    test_history: passedTests,
  }
}
